#ifndef INQUANTIZATION_HPP_
#define INQUANTIZATION_HPP_
 ...
 */
class INQuantization {
public:
  explicit INQuantization(string model, string test_model, string weights, string model_quantized,
    int iterations, string trimming_mode, string gpus, string solver);
  void INQuantizeNet();
  void INQuantizeTest();
  void INQ(caffe::shared_ptr<Net<float> > caffe_net, float per);

private:
  void CheckWritePermissions(const string path);
  void CopyLayers(caffe::Solver<float>* solver, const std::string& model_list);
  void Trim2FixedPoint_cpu(float* data,
      const int cnt, const float thread, const int fl,
      float* mask);
  void Trim2IntegerPowerOf2_cpu(float* data,
