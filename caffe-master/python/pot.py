...
def main(args):
    ...
    if args.output_model is None:
        ...
        args.output_model = file_name + '_power_of_two.prototxt'
    if args.output_weights is None:
        file_name = osp.splitext(args.weights)[0]
        args.output_weights = file_name + '_power_of_two.caffemodel'
    width_data = 8
    ...
    for j, layer in enumerate(model.layer):
        if 'Convolution' in layer.type or 'InnerProduct' in layer.type:
            ...
            if 'Convolution' in layer.type : layer.type = 'ConvolutionInq' 
            if 'InnerProduct' in layer.type and layer.inner_product_param.num_output != 1000 : layer.type = 'FcInq'
            layer.quantization_param.precision = 0
            layer.quantization_param.bw_layer_in = width_data
            layer.quantization_param.fl_layer_in = width_data - fl_layer_in
            layer.quantization_param.bw_layer_out = width_data
            layer.quantization_param.fl_layer_out = width_data - fl_layer_out
