...
import merged_bn_scale
import merged_scale_conv
import incremental_quantization
import pot
import option
...
def main(args):
    ...
    print model_first,weight_first
        
   
    kargs, model_second, weight_second = create_args(model_first,weight_first,".Second")
    merged_bn_scale.main(kargs)

    print "Second phase complete!"
    
    kargs, model_third, weight_third = create_args(model_second,weight_second, ".Third")
    kargs.solver = args.solver_second; kargs.gpu = str(args.gpu); kargs.mode = args.mode
    incremental_quantization.main(kargs)
    
    print "Third phase complete!"
    
    kargs, model_fourth, weight_fourth = create_args(model_second,weight_third, ".fourth")
    merged_scale_conv.main(kargs)
    kargs, model_final, weight_final = create_args(model_fourth,weight_fourth, None, file_name+ '.Final.prototxt', file_name+ '.Final.caffemodel')
    pot.main(kargs)
    
    print "All phases complete, end fixed-point quantization process"
        
    net = caffe.Net(model_final,weight_final,caffe.TEST)
